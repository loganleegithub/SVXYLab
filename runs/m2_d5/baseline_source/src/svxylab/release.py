...
def runtime_config(config, cutoff):
    ...
    requested=config['data']['locked_historical_start']
    schedule=xcals.get_calendar('XNYS',start=requested,end=(pd.Timestamp(requested)+pd.Timedelta(days=15)).date().isoformat()).schedule
    adapted['data']['outer_test_requested_start']=schedule.index[0].strftime('%Y-%m-%d')
...
def verify_release(root):
    ...
    mechanism_amendment=root/'runs/m2_mechanism/runtime_compatibility.json'
    if mechanism_amendment.exists():
        change=read_json(mechanism_amendment)
        if (change['p7_freeze_sha256']!=digest(root/'runs/p7/freeze.json')
                or not amendment.exists() or change['n1_amendment_sha256']!=digest(amendment)):
            raise ValueError('M2诊断兼容记录引用的历史版本不符')
        for path,item in change['changed_frozen_files'].items():
            if sources.get(path)!=item['before']:
...
def economic_supplements(root, config, predictions, account_inputs, output):
    ...
    scored_combined=pd.concat([predictions[m].assign(model=m) for m in ['M0','M1','M2']],ignore_index=True)
    accepted=read_json(root/'runs/p4/acceptance.json')
    prior=pd.read_csv(root/accepted['accepted_prediction_file'],float_precision='round_trip')
    prior=prior.loc[prior.decision_session.lt(scopes['full'])].groupby('model',sort=False).tail(config['clock']['extra_delay_sensitivity_sessions'])
    prior.to_csv(output/'pre_2024_execution_bridge.csv',index=False)
    source_dir=Path(accepted['accepted_prediction_file']).parent
    bridge_models={str(source_dir/'models'/(fit+'.json')):digest(root/source_dir/'models'/(fit+'.json')) for fit in prior.loc[prior.forecast_available,'fit_id'].unique()}
    write_json(output/'bridge_model_references.json',{'source_prediction_file':accepted['accepted_prediction_file'],'source_prediction_sha256':accepted['accepted_prediction_sha256'],'model_files_sha256':bridge_models,'purpose':'Original accepted P4 forecasts needed for first actual delayed execution; excluded from P7 prediction scores.'})
    combined=pd.concat([prior,scored_combined],ignore_index=True)
    ...
    risk=risk_diagnostics(prices,scored_combined,.05,.0005)
...
def run_release(root, output):
    ...
    for variant in specs:
        ...
        _,variant_result=run_variant(frame,config,variant,directory)
    ...
    supplements=economic_supplements(root,config,{m:predictions[m] for m in ['M0','M1','M2']},account_inputs,output)
