...
def test_runtime_adapter_changes_only_evaluation_bounds(synthetic_prediction_data):
    ...
    assert config==old
    assert c['data']['outer_test_requested_start']=='2020-01-02'  # Jan 1 is a holiday.
...
def test_delayed_first_execution_needs_prior_accepted_forecast():
    from svxylab.economics import model_targets
    sessions=['2023-12-28','2023-12-29','2024-01-02']
    def p(day,decision,mu,q):return {'as_of_session':day,'decision_session':decision,'model':'M2','forecast_available':True,'mu5':mu,'q90':q,'fit_id':day+'_M2','simulation_decision_at':decision+'T19:00:00Z'}
    current=pd.DataFrame([p('2023-12-29','2024-01-02',.02,.1)])
    prior=pd.DataFrame([p('2023-12-28','2023-12-29',.01,.2)])
    before,_=model_targets(current,sessions,.05,.0005)
    after,_=model_targets(pd.concat([prior,current]),sessions,.05,.0005)
    assert pd.isna(before['M2_main'].shift(1).loc['2023-12-29'])
    assert after['M2_main'].shift(1).loc['2023-12-29']==pytest.approx(.25)
    assert after['M2_main'].loc['2023-12-29']==before['M2_main'].loc['2023-12-29']==pytest.approx(.5)
