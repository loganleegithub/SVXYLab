...
def audit_ledgers(output, prices, curve, predictions):
    ...
    bridge=pd.read_csv(output/'pre_2024_execution_bridge.csv',float_precision='round_trip')
    pred={n:pd.concat([bridge.loc[bridge.model.eq(n)],p],ignore_index=True).set_index('as_of_session') for n,p in predictions.items()};maximum=0.;count=0
    for item in specs:
