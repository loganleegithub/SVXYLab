...
M0/M1/M2各main/risk-only及6个简单对照完整交付，基础B=.05、单边c=.0005；只做原定单因素敏感性。完整请求账户1006日，前175日模型无服务；共同起点2020-09-11后831日，全部账户同资本、同一实际收盘时点从现金重建。缺预测保持实际份额和现金；P4最后5个跨2024标签的预测仍用于2023账户，不读2024收益。共同起点M2 main净收益104.33%、最大回撤32.98%，固定100%为194.02%、29.18%；三个模型的μ5过滤均降低本段总收益，较差结果保留。当前P5完成待验收，未执行P6、封存评价或模型晋升。全部细节见 STATUS.md。
...
正式 ETF 仅用 Yahoo Chart 原件；`provider_*` 为供应商值，OHLCV 当时份额复原字段有明确标记。保留 Nasdaq/QC 参考差异以及 P2 的 SPY 分红总回报与供应商复权参考差异。当前停在 P5，验收后再继续。P0 检查点 `f3cc00f`、P1 检查点 `dc3d4ff`、P2 检查点 `5912f3b`、P3 检查点 `541178b`、P4 检查点 `de8b43d`；已有origin但本轮未使用，未改全局 Git 配置。验收只代表交付完整、真实与可复算，不代表模型有收益优势。M1/M2 在当前开发段的主要平均预测损失未优于 M0，原结果保留。
