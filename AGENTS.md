...
M3相对M2的MSE/对数损失/Pinball变化为-1.42%/+1.08%/+3.39%；SKEW相对相同样本核心M2为+2.29%/+1.09%/+2.15%。部分删列改善条件性指标，全部正负结果保留；列消融不是原始来源的因果证明。当前停在P6待验收，未执行2024以后封存评价、P7、账户连接或订单；下一阶段主版本仍是原预定M2，不自动晋升。详见 STATUS.md。
...
正式 ETF 仅用 Yahoo Chart 原件；`provider_*` 为供应商值，OHLCV 当时份额复原字段有明确标记。保留 Nasdaq/QC 参考差异以及 P2 的 SPY 分红总回报与供应商复权参考差异。当前停在 P6，验收后再继续。P0 检查点 `f3cc00f`、P1 检查点 `dc3d4ff`、P2 检查点 `5912f3b`、P3 检查点 `541178b`、P4 检查点 `de8b43d`、P5 验收检查点 `a10575c`；已有origin但本轮未使用，未改全局 Git 配置。验收只代表交付完整、真实与可复算，不代表模型有收益优势。M1/M2 在当前开发段的主要平均预测损失未优于 M0，原结果保留。
