...
**P3 原始联合特征已实现；尚未建立联合模型或证明收益优势。** 真实数据覆盖 2019-01-02 至 2026-09-04 的 1930 个股票交易日；预热后 19 项核心共同完整 1909 日，加 SKEW 后 1905 日。查看 `reports/features.html` 的公式、覆盖、状态卡与信息重叠；阶段进度见 STATUS.md。
...
1. 双击 `reports/features.html` 查看 19 项核心特征、独立 SKEW 扩展、三张真实状态卡和核对结果。
2. 核对 STATUS.md 后验收 P3；本轮停在此阶段。
3. 重新计算使用 `.venv/bin/python -m svxylab features --open`，只读取已验收的冻结数据。实际命令和普通测试说明见 AGENTS.md。
...
- reports/features.html：P3 特征报告；reports/baselines.html、reports/data.html、reports/environment.html 保留 P2/P1/P0 快照。
- data/raw / data/clean：原始响应与清洗 CSV；下载时间、来源和 SHA-256 在 data/raw/downloads.jsonl。
- runs/p0 / runs/p1 / runs/p2 / runs/p3：实际命令、失败、核验、冻结和实验变更记录。
...
P0 本地 Git 检查点为 `f3cc00f`，P1 为 `dc3d4ff`，P2 为 `5912f3b`，无远端。P2 页面净值与汇总、P3 相关性只展示 2019–2023，全段 CSV 保留；封存模型评价仍属 P7。当前目录没有 START_HERE.html 或 inputs/reference；未重建或声称读取缺失附件。
