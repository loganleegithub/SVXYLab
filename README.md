...
**P2 总回报、时钟、五日标签与连续账本已实现；尚未建立联合模型或证明收益优势。** 核心真实数据覆盖 2019-01-02 至 2026-09-04 的全部 1930 个股票交易日。查看 `reports/baselines.html` 的预定对照、三段真实手算和测试；阶段进度见 STATUS.md。
...
1. 双击 `reports/baselines.html` 查看六个对照、实际损益账本、公司行动和标签时钟。
2. 核对 STATUS.md 后验收 P2；本轮停在此阶段。
3. 重新计算使用 `.venv/bin/python -m svxylab baselines --open`，只读取已验收的冻结数据。实际命令和普通测试说明见 AGENTS.md。
...
- reports/baselines.html：P2 账本报告；reports/data.html 和 reports/environment.html 保留 P1/P0 快照。
- data/raw / data/clean：原始响应与清洗 CSV；下载时间、来源和 SHA-256 在 data/raw/downloads.jsonl。
- runs/p0 / runs/p1 / runs/p2：实际命令、失败、核验、冻结和实验变更记录。
...
P0 本地 Git 检查点为 `f3cc00f`，P1 已验收检查点为 `dc3d4ff`，无远端。P2 页面净值与汇总只展示 2019–2023，全段 CSV 保留，2024 年后仅用于会计/时钟核验；封存模型评价仍属 P7。当前目录没有 START_HERE.html 或 inputs/reference；未重建或声称读取缺失附件。
